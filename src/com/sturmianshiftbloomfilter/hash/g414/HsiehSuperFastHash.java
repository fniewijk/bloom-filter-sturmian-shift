...
public class HsiehSuperFastHash implements LongHash {
    private final static int[] LEFT_SHIFT_WIDTHS = { 0, 10, 11, 16, 43, 42, 43, 48 };
    private final static int[] RIGHT_SHIFT_WIDTHS = { 0, 1, 17, 11, 49, 33, 49, 43 };
    ...
    public long[] getLongHashCodes(final String object, final int k) {
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                final long newHash = computeHsiehLongHash(representation, lastHash);
                hashCodes[i] = newHash;
                lastHash = newHash;
            }
    ...
    public int[] getIntHashCodes(final String object, final int k) {
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                final int newHash = computeHsiehIntHash(representation, lastHash);
                hashCodes[i] = newHash;
                lastHash = newHash;
            }
     ...
     * @return
     */
    public long computeHsiehLongHash(final byte[] data, final long seed) {
        if (data == null) {
            return seed;
        }
        ...
        for (int i = 0; i < len - 8; i += 8) {
            ...
            final long tmp = LongHashMethods.gatherIntLE(data, i + 4) << 27;
            hVal = (hVal << 32) ^ tmp;
            hVal += hVal >> 43;
        }
     ...
     * @return
     */
    public static int computeHsiehIntHash(final byte[] data, final int seed) {
        if (data == null) {
            return seed;
        }
        ...
        for (int i = 0; i < len - 4; i += 4) {
            ...
            final int tmp = LongHashMethods.gatherPartialIntLE(data, i + 2, 2) << 11;
            hVal = (hVal << 16) ^ tmp;
            hVal += hVal >> 11;
        }
